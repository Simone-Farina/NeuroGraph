...
export async function createCrystalAndProcessEdges(
  ...
  if (similarError) {
    throw new Error(similarError.message);
  }
  ...
  if (relatedCrystalsInput.length > 0) {
    ...
    if (requestedTargetIds.length > 0) {
      ...
      if (relatedTargetsError) {
        throw new Error(relatedTargetsError.message);
      }
  ...
  if (mediumSuggestions.length > 0) {
    ...
    if (existingEdgesError) {
      throw new Error(existingEdgesError.message);
    }
  ...
  return { crystal, edges: createdEdges, edge_suggestions: edgeSuggestions };
}
